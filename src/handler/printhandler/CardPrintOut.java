 ...
 *
 */
public class CardPrintOut {// TODO ad constructor for settings, make settings
	// change printouts
	 ...
	 * @param computer
	 *            is your opponent. in this version computer type Player
	 * @param drawingStyle
	 *            is drawing style
	 *
	 */
	...
	public static String createStringRepresentationOfOfPlayersHandSecondStyle(List<Card> playerHand){
		StringBuilder sb = new StringBuilder();
		List<String> valueStrings = new ArrayList<>();
		List<String> colorStrings = new ArrayList<>();
		...
		if (middleSectionText == null) {
			...
			if (handSize > 1) {
				sb.insert(0,createLeftSideOfLine(sizePartial, leftBorder, middleSection,
						null));
				// append after first left side
				sb.insert(sb.length() - sizeX / 2, createLeftSideOfLine(sizeX / 2, join, middleSection,
						null));
			} else {
